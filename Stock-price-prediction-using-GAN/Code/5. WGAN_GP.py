...
class GAN():
    ...
    def train(self, X_train, y_train, yc, epochs):
        ...
        return Predicted_price, Real_price, np.sqrt(mse(Real_price, Predicted_price).numpy()) / np.mean(Real_price)
...
RMSE = np.sqrt(MeanSquaredError()(np.array(predicted), np.array(real)).numpy())
print('-- RMSE -- ', RMSE)
